...
def train(mazes):
    ...
    for episode in range(1, EPISODES + 1):
        ...
        maze_success_rates = [
            ...
            for wins in maze_recent_wins
        ]
        weakest_success = min(maze_success_rates)
        average_success = sum(maze_success_rates) / len(maze_success_rates)
        score = weakest_success + 0.01 * average_success
        ...
        if episode % 50 == 0:
            ...
            for maze_index in range(len(mazes)):
                ...
                print(
                    ...
                    f"positive_weight={reward_weight:.2f}"
                )

    model.load_state_dict(best_state)
    return model, best_episode, best_score
...
def main():
    ...
    torch.save(model.state_dict(), "ppo_multi_maze_best.pt")

    print_training_parameters()
    print(f"loaded_best_episode={best_episode}")
    print(f"loaded_best_score={best_score:.3f}")

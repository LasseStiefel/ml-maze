...
SAC Maze Solver
===============
Master's Degree — Machine Learning Project

Algorithm: SAC (Soft Actor-Critic) via stable-baselines3
- Off-policy actor-critic with entropy regularisation
...
Action space: 4 continuous scores — argmax picks the discrete direction.
Observation and rewards come from the shared MazeEnv (same as all other agents).

Select the maze via MAZE_ID in config.py (1, 2, or 3).

Run:
    python ddpg_train.py              # full training + test
    python ddpg_train.py --test-only  # reproduce test without retraining

Outputs (saved to Results/):
    sac_model_maze<N>.zip          — final trained model
    sac_best_maze<N>.zip           — best checkpoint by eval reward
    training_SAC_maze<N>.csv       — per-episode stats
    training_plot_SAC_maze<N>.png  — learning curves
"""
...
def save_training_csv(training_history: list) -> None:
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['episode', 'timestep', 'eval_reward', 'eval_steps',
                         'final_dist', 'min_dist', 'train_reward', 'solved'])

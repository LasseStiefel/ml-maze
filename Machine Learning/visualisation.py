"""
Shared visualisation utilities for DQN, DQN2, SAC, and SARSA maze agents.
...
Provides:
    ...
    save_dqn_training_plot()   — 2×2 DQN (TensorFlow) learning curve figure
    save_dqn2_training_plot()  — 2×2 DQN2 (PyTorch) learning curve figure
...
# ---------------------------------------------------------------------------
# DQN training plot  (reward / steps / loss / epsilon)
# ---------------------------------------------------------------------------
def save_dqn_training_plot(
    rewards:   list,
    steps:     list,
    losses:    list,
    epsilons:  list,
    ...
    """Save a 2×2 figure showing DQN learning curves."""
    fig, axes = plt.subplots(2, 2, figsize=(14, 8))
    fig.suptitle(f'DQN Training — Maze {maze_id} ({size_label})', fontsize=14)
    ...
    plt.tight_layout()
    out = os.path.join(results_dir, f'training_plot{suffix}.png')
...
# ---------------------------------------------------------------------------
# DQN2 training plot  (reward / steps / loss / epsilon)  — PyTorch variant
# ---------------------------------------------------------------------------
def save_dqn2_training_plot(
    rewards:   list,
    steps:     list,
    losses:    list,
    epsilons:  list,
    ...
    """Save a 2×2 figure showing DQN2 (PyTorch) learning curves."""
    fig, axes = plt.subplots(2, 2, figsize=(14, 8))
    fig.suptitle(f'DQN2 (PyTorch) Training — Maze {maze_id} ({size_label})', fontsize=14)
    ...
    ax.plot(eps_range, _rolling_mean(losses), color='green',
            linewidth=2, label='50-ep avg')
    ax.set_title('Average MSE loss per episode')
    ax.set_xlabel('Episode'); ax.set_ylabel('Loss')
    ax.legend(); ax.grid(alpha=0.3)
    ...
    plt.tight_layout()
    out = os.path.join(results_dir, f'training_plot_DQN2{suffix}.png')
    plt.savefig(out, dpi=150)
    plt.close()
    print(f'Plot saved  → {out}')
